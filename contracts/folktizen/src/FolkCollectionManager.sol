...
contract FolkCollectionManager {
    ...
    function _checkToken(
        address token,
        uint256 price,
        FolkLibrary.CollectionType collectionType,
        bool useAgent
    ) internal view {
        uint256 _base = accessControls.getTokenBase(token);
        if (
            (collectionType == FolkLibrary.CollectionType.IRL &&
                price < _base) ||
    ...
    function _checkArtistOrAgentOwner(
        ...
        if (
            agentId > 0 && skypodAgentManager.getIsAgentWallet(_artist, agentId)
        ) {
    ...
    function getCollectionType(
        uint256 collectionId
    ) public view returns (FolkLibrary.CollectionType) {
    ...
    function setSkypodAgentManager(
        address payable _skypodAgentManager
    ) external onlyAdmin {
        skypodAgentManager = SkypodAgentManager(_skypodAgentManager);
    }

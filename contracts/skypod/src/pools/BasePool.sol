...
abstract contract BasePool is IPool {
    ...
    SkypodUserManager public userManager;
    ...
    function claimCycleRewards() external override {
        if (_userBalances[msg.sender] == 0 || _userRewards[msg.sender] == 0) {
            revert SkypodErrors.NoCycleRewards();
        }
    ...
    function cleanCycle(uint256 cycle) external override onlyAdmin {
        ...
        if (IERC20(novo).balanceOf(address(this)) >= _amount) {
            ...
            emit CycleCleaned(novo, cycle, _amount);
        } else {
            revert SkypodErrors.InsufficientCycleBalance();
        }
